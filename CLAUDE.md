...
- `lib/models.ts` — `Post`, `Project`, and `Experience` schemas (all have `published`; projects/experiences have `sortOrder`). Experience fallback lives in `data/experience.ts` and renders in the About section's Work Experience timeline.
